...
import ProjectArchitectureView from "../components/architecture/ProjectArchitectureView";
    ...
    <div className="csd-slide-content csd-cover" key={animKey}>
      ...
      <h1 className="csd-cover-title">
        {project.name.split("").map((char, i) => (
          <span key={i} className="csd-cover-char" style={{ animationDelay: `${i * 0.04}s` }}>
            {char === " " ? "\u00A0" : char}
          </span>
        ))}
      </h1>
...
function PurposeSlide({ project, animKey }: { project: Project; animKey: number }) {
  return (
    <div className="csd-slide-content" key={animKey}>
      <div className="csd-slide-eyebrow"><span className="csd-eyebrow-line" />The Problem</div>
...
function TechSlide({ project, animKey }: { project: Project; animKey: number }) {
  return (
    <div className="csd-slide-content" key={animKey}>
      <div className="csd-slide-eyebrow"><span className="csd-eyebrow-line" />Technology</div>
...
function UniquenessSlide({ project, animKey }: { project: Project; animKey: number }) {
  return (
    <div className="csd-slide-content" key={animKey}>
      <div className="csd-slide-eyebrow"><span className="csd-eyebrow-line" />Differentiator</div>
...
function ImpactSlide({ project, animKey }: { project: Project; animKey: number }) {
  return (
    <div className="csd-slide-content" key={animKey}>
      <div className="csd-slide-eyebrow"><span className="csd-eyebrow-line" />Outcomes</div>
...
export default function CaseStudyDetail() {
  ...
  const goTo = useCallback((index: number) => {
    ...
    animTimeout.current = setTimeout(() => { setAnimating(false); setPrevSlide(null); }, 600);
  ...
  }, [currentSlide, animating, goTo, navigate, archMode]);
  ...
  return (
    <>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .csd-root {
          ...
          overflow: hidden; z-index: 9999; cursor: default;
        }
        ...
        .csd-geo { position: absolute; z-index: 2; pointer-events: none; will-change: transform; transition: transform 0.12s ease-out; }
        .csd-geo-1 { right: 8%;  top: 15%; width: 320px; height: 320px; border: 1px solid rgba(var(--accent-rgb), 0.08); border-radius: 50%; transform: translate(${px2 * 1.4}px, ${py2 * 1.4}px); }
        .csd-geo-2 { right: 12%; top: 20%; width: 200px; height: 200px; border: 1px solid rgba(var(--accent-rgb), 0.05); border-radius: 50%; transform: translate(${px2 * 0.8}px, ${py2 * 0.8}px); }
        .csd-geo-3 { right: 20%; top: 32%; width: 80px;  height: 80px;  background: rgba(var(--accent-rgb), 0.04); border: 1px solid rgba(var(--accent-rgb), 0.12); border-radius: 16px; transform: translate(${px2 * 0.5}px, ${py2 * 0.5}px) rotate(${px * 0.5}deg); }
        .csd-geo-4 { left: 55%; bottom: 20%; width: 6px; height: 6px; background: var(--accent); border-radius: 50%; opacity: 0.5; transform: translate(${px}px, ${py * 2}px); box-shadow: 0 0 20px 4px rgba(var(--accent-rgb), 0.4); }
        .csd-geo-5 { right: 30%; top: 18%; width: 4px; height: 4px; background: rgba(255,255,255,0.4); border-radius: 50%; transform: translate(${px2 * 2}px, ${py2 * 2}px); }
        ...
        .csd-arch-toggle {
          ...
          backdrop-filter: blur(10px); font-family: 'DM Sans', sans-serif;
        }
        .csd-arch-toggle:hover {
          background: rgba(var(--accent-rgb), 0.16); border-color: rgba(var(--accent-rgb), 0.45);
          transform: translateY(-1px); box-shadow: 0 4px 16px rgba(var(--accent-rgb), 0.2);
        }
        ...
        .csd-dot-row { display: flex; align-items: center; gap: 10px; cursor: pointer; transition: all 0.2s ease; }
        .csd-dot-row:hover .csd-dot-lbl,
        .csd-dot-row.active .csd-dot-lbl { opacity: 1; transform: translateX(0); }
        .csd-dot-lbl {
          font-size: 10px; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase;
          color: rgba(255,255,255,0.5); opacity: 0; transform: translateX(6px); transition: all 0.2s ease;
        }
        .csd-dot {
          width: 5px; height: 5px; border-radius: 50%; background: rgba(255,255,255,0.2);
          transition: all 0.35s cubic-bezier(0.23,1,0.32,1); flex-shrink: 0;
        }
        .csd-dot-row.active .csd-dot {
          background: var(--accent); width: 5px; height: 20px; border-radius: 3px;
        ...
        .csd-counter {
          ...
          font-family: 'Sora', sans-serif; font-size: 11px; font-weight: 700; letter-spacing: 0.12em;
        ...
        .csd-counter-current { font-size: 28px; font-weight: 800; letter-spacing: -0.03em; color: rgba(255,255,255,0.85); line-height: 1; }
        ...
        .csd-click-zone-left  { left: 0;  justify-content: flex-start; padding-left: 28px;  background: linear-gradient(to right, rgba(3,6,15,0.3), transparent); }
        .csd-click-zone-right { right: 0; justify-content: flex-end;   padding-right: 28px; background: linear-gradient(to left,  rgba(3,6,15,0.3), transparent); }
        .csd-zone-arrow {
        ...
        .csd-click-zone:hover .csd-zone-arrow {
          ...
          color: var(--accent); box-shadow: 0 0 20px rgba(var(--accent-rgb), 0.2);
        }
        ...
        .csd-progress-fill {
          height: 100%; background: linear-gradient(90deg, var(--grad-from), var(--grad-to));
          transition: width 0.6s cubic-bezier(0.23,1,0.32,1);
          border-radius: 0 2px 2px 0; box-shadow: 0 0 8px rgba(var(--accent-rgb), 0.6);
        }

        .csd-stage {
          position: absolute; inset: 0; display: flex; align-items: center;
          padding: 100px 120px 100px 10%; z-index: 10; pointer-events: none;
        }

        @keyframes enterNext { 0% { opacity:0; transform:scale(0.92) translateX(40px)  translateY(8px);  filter:blur(4px); } 100% { opacity:1; transform:scale(1) translateX(0)     translateY(0);   filter:blur(0); } }
        @keyframes enterPrev { 0% { opacity:0; transform:scale(0.92) translateX(-40px) translateY(8px);  filter:blur(4px); } 100% { opacity:1; transform:scale(1) translateX(0)     translateY(0);   filter:blur(0); } }
        @keyframes exitNext  { 0% { opacity:1; transform:scale(1)    translateX(0)     translateY(0);    filter:blur(0);   } 100% { opacity:0; transform:scale(1.05) translateX(-30px) translateY(-6px); filter:blur(3px); } }
        @keyframes exitPrev  { 0% { opacity:1; transform:scale(1)    translateX(0)     translateY(0);    filter:blur(0);   } 100% { opacity:0; transform:scale(1.05) translateX(30px)  translateY(-6px); filter:blur(3px); } }
        ...
        .csd-eyebrow-line { display: block; width: 24px; height: 1.5px; background: var(--accent); border-radius: 2px; }
        .csd-slide-title {
          font-family: 'Sora', sans-serif; font-size: clamp(34px, 4.5vw, 56px);
          font-weight: 700; color: #ffffff; line-height: 1.08; letter-spacing: -0.025em; margin-bottom: 28px;
        }
        .csd-stagger-title { animation: staggerUp 0.55s cubic-bezier(0.23,1,0.32,1) 0.08s both; }
        .csd-slide-body { font-size: 16px; line-height: 1.8; color: rgba(255,255,255,0.62); max-width: 520px; font-weight: 300; }
        .csd-stagger-body { animation: staggerUp 0.55s cubic-bezier(0.23,1,0.32,1) 0.16s both; }
        @keyframes staggerUp { from { opacity:0; transform:translateY(18px); } to { opacity:1; transform:translateY(0); } }

        .csd-cover-eyebrow {
          display: flex; align-items: center; gap: 8px;
          font-size: 10px; font-weight: 700; letter-spacing: 0.18em; text-transform: uppercase;
          color: var(--accent); margin-bottom: 20px; animation: staggerUp 0.5s ease 0.05s both;
        }
        .csd-cover-cat-dot {
          width: 5px; height: 5px; border-radius: 50%; background: var(--accent);
          box-shadow: 0 0 8px rgba(var(--accent-rgb), 1); animation: dotPulse 2.5s ease-in-out infinite;
        }
        @keyframes dotPulse {
        ...
        .csd-cover-title {
          font-family: 'Sora', sans-serif; font-size: clamp(40px, 7vw, 80px);
          font-weight: 600; line-height: 0.95; letter-spacing: -0.035em;
          margin-bottom: 36px; padding-bottom: 4px; display: block; white-space: nowrap;
        }
        .csd-cover-char {
          display: inline-block;
          background: linear-gradient(140deg, #ffffff 0%, rgba(var(--accent-rgb), 0.75) 100%);
          -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
          opacity: 0; padding: 10px 2px;
          animation: charIn 0.5s cubic-bezier(0.23,1,0.32,1) forwards;
        }
        @keyframes charIn { from { opacity:0; transform:translateY(20px) skewY(3deg); } to { opacity:1; transform:translateY(0) skewY(0); } }
        .csd-cover-tagline { font-size: 16px; line-height: 1.7; color: rgba(255,255,255,0.55); max-width: 460px; margin-bottom: 28px; font-weight: 300; animation: staggerUp 0.5s ease 0.28s both; }
        .csd-cover-tags { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 36px; }
        .csd-cover-tag {
          font-size: 11px; font-weight: 500; color: rgba(255,255,255,0.45);
          background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1);
          padding: 5px 13px; border-radius: 6px; opacity: 0;
          animation: staggerUp 0.5s cubic-bezier(0.23,1,0.32,1) forwards;
        }
        .csd-cover-hint { display: flex; align-items: center; gap: 7px; font-size: 11px; color: rgba(255,255,255,0.22); animation: staggerUp 0.5s ease 0.5s both; }

        .csd-tech-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; max-width: 560px; }
        .csd-tech-item {
          background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07);
          border-radius: 14px; padding: 18px 20px; opacity: 0;
          animation: staggerUp 0.5s cubic-bezier(0.23,1,0.32,1) forwards;
        ...
        .csd-impact-grid { display: flex; flex-direction: column; gap: 0; }
        .csd-impact-item { padding: 20px 0; opacity: 0; animation: staggerUp 0.55s cubic-bezier(0.23,1,0.32,1) forwards; position: relative; }
        .csd-impact-metric {
        ...
        .csd-impact-rule  { position: absolute; bottom: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, rgba(255,255,255,0.07), transparent); }

        .csd-arch-placeholder { display: flex; flex-direction: column; align-items: center; text-align: center; padding: 48px; max-width: 440px; margin: 0 auto; }
        .csd-arch-rings { position: relative; width: 120px; height: 120px; display: flex; align-items: center; justify-content: center; margin-bottom: 32px; }
        .csd-arch-ring  { position: absolute; border-radius: 50%; border: 1px solid; animation: ringPulse 3s ease-in-out infinite; }
        ...
        .react-flow__node {
          ...
          color: inherit !important; width: auto !important;
        }
        .react-flow__node.selected,
        .react-flow__node:focus,
        .react-flow__node:focus-visible { outline: none !important; box-shadow: none !important; }
        .react-flow__controls button { background: rgba(18,18,24,0.9) !important; border-color: rgba(255,255,255,0.08) !important; color: rgba(255,255,255,0.5) !important; }
        .react-flow__controls button:hover { background: rgba(30,30,40,0.95) !important; color: rgba(255,255,255,0.9) !important; }
      ...
      <div
        className={`csd-root ${!archMode && cursorZone === "left" ? "cursor-left" : !archMode && cursorZone === "right" ? "cursor-right" : ""}`}
        onMouseMove={handleMouseMove}
        onClick={handleClick}
      >
        {/* ── Background (always visible) ── */}
        <div className="csd-bg">
          {prevBgImg && <div className="csd-bg-img" style={{ backgroundImage: `url(${prevBgImg})`, opacity: 0 }} />}
          <div className="csd-bg-img" style={{ backgroundImage: `url(${bgImage})`, opacity: imgLoaded[currentSlide] ? 1 : 0, transform: `translate(${px * 0.4}px, ${py * 0.4}px) scale(1.08)` }} />
          <div className="csd-bg-glow" />
          <div className="csd-grain" />
        </div>

        {/* ── Geometric parallax (always visible) ── */}
        ...
        <div className="csd-topbar">
          <button className="csd-back-btn" onClick={(e) => { e.stopPropagation(); navigate(-1); }}>
          ...
          <button className="csd-arch-toggle" onClick={(e) => { e.stopPropagation(); setArchMode(v => !v); }}>
            {archMode ? (
              <><svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M9 2L4 6.5 9 11" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" /></svg>Back to Story</>
            ) : (
              <><svg width="13" height="13" viewBox="0 0 13 13" fill="none"><rect x="1" y="4.5" width="4" height="4" rx="1.2" stroke="currentColor" strokeWidth="1.3" /><rect x="4.5" y="1" width="4" height="4" rx="1.2" stroke="currentColor" strokeWidth="1.3" /><rect x="8" y="4.5" width="4" height="4" rx="1.2" stroke="currentColor" strokeWidth="1.3" /></svg>Architecture View</>
            )}
          </button>
        </div>
        ...
        {/* ── Story layer ── */}
        {!archMode && (
          <div className="csd-story-layer">
            {/* Click zones */}
            <div className={`csd-click-zone csd-click-zone-left ${currentSlide === 0 ? "disabled" : ""}`} onClick={() => goTo(currentSlide - 1)}>
            ...
            <div className={`csd-click-zone csd-click-zone-right ${currentSlide === SLIDES.length - 1 ? "disabled" : ""}`} onClick={() => goTo(currentSlide + 1)}>
              <div className="csd-zone-arrow">
            ...
            <div className="csd-nav-dots">
              {SLIDES.map((slide, i) => (
                <div key={slide.id} className={`csd-dot-row ${i === currentSlide ? "active" : ""}`} onClick={(e) => { e.stopPropagation(); goTo(i); }}>
            ...
            {/* Counter */}
            <div className={`csd-counter ${counterVisible ? "visible" : ""}`} onMouseEnter={() => setCounterVisible(true)} onMouseLeave={() => setCounterVisible(false)}>
            ...
            <div className="csd-progress-track">
              <div className="csd-progress-fill" style={{ width: `${((currentSlide + 1) / SLIDES.length) * 100}%` }} />
            </div>
          </div>
        )}

        {/* ── Architecture overlay (completely separate DOM layer) ── */}
        {archMode && (
          <div className="csd-arch-overlay">
            {project.architectureData
              ? <ProjectArchitectureView data={project.architectureData} accentColor={project.accentColor} />
